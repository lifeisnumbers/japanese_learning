...
const KanjiFlashcard = () => {
  ...
  useEffect(() => {
    const loadNotRememberedList = async () => {
      const storedList = await AsyncStorage.getItem('notRememberedListKanji');
      if (storedList) setNotRememberedList(JSON.parse(storedList));
    };
    loadNotRememberedList();
  }, []);
  useEffect(() => {
    AsyncStorage.setItem('notRememberedListKanji', JSON.stringify(notRememberedList));
  }, [notRememberedList]);

  const fetchVocabulary = (lesson) => {
    fetch(`https://api-japan-2.vercel.app/api/kanji/getByBai?bai=${lesson}`)
    ...
    <View style={styles.container}>
      ...
      {data.length > 0  && data[currentCardIndex] ?(
        <TouchableOpacity onPress={flipCard} style={styles.card}>
          {!isFlipped ? (
            <Text style={styles.textkanji}>{data[currentCardIndex].Kanji}</Text>
          ) : (
            <View style={styles.kanjicontainer}>
              <Text style={styles.text}>{data[currentCardIndex].Onyomi}</Text>
              <Text style={styles.text}>{data[currentCardIndex].Kunyomi}</Text>
              <Text style={styles.text}>{data[currentCardIndex].Nghia}</Text>
              <Text style={styles.text}>{data[currentCardIndex].Hantu}</Text>
            </View>
          )}
        </TouchableOpacity>
      ) : (
