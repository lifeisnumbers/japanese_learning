...
import { View, Text, TextInput, Button, StyleSheet, TouchableOpacity, Switch,Modal } from 'react-native';
...
const MinaTracN = () => {
  ...
  const [currentTimer, setCurrentTimer] = useState(initialTimerDuration); 
  useEffect(() => {
    let countdown = null;
    ...
    fetch(`https://api-japan-2.vercel.app/api/tuvung/getByBai?bai=${selectedLesson}`)
      ...
      .then(data => {
        let shuffledQuestions = data.map(item => ({
          question: item.ja,
          correctAnswer: item.romaji, // or item.nghia for meaning
          allAnswers: shuffleArray([item.romaji, ...getRandomAnswers(data, item.romaji)])
        }));
  ...
  const getRandomAnswers = (data, correctAnswer) => {
    ...
    while (answers.length < 3) {
      const randomAnswer = data[Math.floor(Math.random() * data.length)].romaji; // or .nghia
